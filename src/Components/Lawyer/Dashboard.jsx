...
const TABLE_HEAD = ["Name", "Date", "Time", "Status"];


function Dashboard() {
   const { isLoading: appointmentDataLoading, error: appointmentDataError, data: appointmentData } = useQuery(
        ['appointments'],
        () => LawyerRequest.get(`/appointmentrequest?date=${moment().format('YYYY-MM-DD')}`).then((res) => res.data),
        { retry: false }
    );

      const { isLoading: dashboardDataLoading, error: dashboardDataError, data: dashboardData } = useQuery(
        ['dashboardData'],
        () => LawyerRequest.get(`/dashboarddata`).then((res) => res.data),
        { retry: false }
    );
            ...
            {/* <div className="flex flex-col lg:flex-row w-full">
              <div className="lg:w-full h-40 m-5 lg:m-5 shadow-xl rounded-xl flex justify-evenly items-center">
                <Card className="w-1/5 h-4/5">
                  <div className="w-2/4 h-full bg-green-800 rounded-xl"></div>
                </Card>
                <Card className="w-1/5 h-4/5">
                  <div className="w-2/4 h-full bg-blue-800 rounded-xl"></div>
                </Card>
                <Card className="w-1/5 h-4/5">
                  <div className="w-2/4 h-full bg-yellow-800 rounded-xl"></div>
                </Card>
                <Card className="w-1/5 h-4/5">
                  <div className="w-2/4 h-full bg-orange-800 rounded-xl"></div>
                </Card>
              </div>
            </div> */}
                  ...
                  <table className="w-full min-w-max table-auto text-left">
                    ...
                    <tbody>
                      {appointmentData?.data.map((appointment, index) => {
                        const isLast =
                          index === appointmentData.data.length - 1;
